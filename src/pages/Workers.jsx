...
export default function Workers() {
  ...
  function openEdit(w) { setForm(w); setEditingId(w.id); setShowForm(true) }

  async function handleSave(e) {
    e.preventDefault()
    if (editingId) await supabase.from('workers').update(form).eq('id', editingId)
    else await supabase.from('workers').insert(form)
    setShowForm(false)
    load()
  }
  ...
  return (
    <div>
      <PageHeader
        title="Công nhân"
        subtitle="Danh sách nhân viên trong xưởng."
        action={
            ...
            {workers.map((w) => (
              <tr key={w.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3 text-right space-x-3">
                  ...
                  <button onClick={() => handleDelete(w.id)} className="text-rose-600 hover:underline">Xóa</button>
                </td>
              </tr>
            ))}
            {!loading && workers.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-400">Chưa có công nhân nào.</td></tr>
            )}
          </tbody>
        </table>
      </Card>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-navy-950/50 flex items-center justify-center z-50 px-4">
          <form onSubmit={handleSave} className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4">
            ...
            {[
              ['full_name', 'Họ tên'], ['employee_code', 'Mã nhân viên'], ['department', 'Bộ phận'],
              ['position', 'Chức vụ'], ['phone', 'Số điện thoại'],
            ].map(([key, label]) => (
              <div key={key}>
                <label className="block text-sm font-medium text-navy-700 mb-1">{label}</label>
                <input required={key === 'full_name'} value={form[key] || ''}
